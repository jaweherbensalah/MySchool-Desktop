...
import static StudentServices.ListServicesController.i;
...
public class PaymentController implements Initializable {
   ...
   // public static int ze;
    ...
    public void initialize(URL url, ResourceBundle rb) {
     ...
     public void calculate(){
        try {
            ...
            String query="SELECT sum(service.price*quantity) "
                    + "as totalamount FROM orders JOIN service"
                    + " ON "
                    + "orders.menu_id=service.menu_id"
                    + " WHERE orders.id="+i+" and order_status='CONFIRMED'";
    ...
    public void cashOnDelivery(ActionEvent event){
        ...
        String query= "UPDATE payment SET payment.payment_type='CASH_ON_DELIVERY"
                + "' WHERE payment.payment_status='NOT_CONFIRMED' and order_id IN"
                + "(SELECT order_id from orders WHERE orders.order_id=payment.order_id and id=?)";
    ...
    public void onlinePayment(ActionEvent event){
        ...
        String query="UPDATE payment SET payment.payment_type='ONLINE_PAYMENT'"
                + " WHERE payment.payment_status='NOT_CONFIRMED' and order_id IN(SELECT order_id from orders "
                + "WHERE orders.order_id=payment.order_id and id=?)";
         PreparedStatement pst;
         try {
             pst =con.prepareStatement(query);
             pst.setInt(1, i);
         ...
         try {
             if(paymentModel.isPaymentOnline()){
                 if(!(cardnumtxt.getText().isEmpty()||cardholdertxt.getText().isEmpty()||cvvtxt.getText().isEmpty()||monthtxt.getText().isEmpty()||yeartxt.getText().isEmpty())){
                     ...
                     if(!(cardnumtxt.getText().length()==10)){
                         warning.setText("Numéro de carte invalide !");
                         return;
                     }else if(!(cvvtxt.getText().length()==3)){
                         warning.setText(" CVV invalide !");
                         return;
                     }else if(!((monthtxt.getText()).matches("0?[1-9]|1[012]"))){
                         warning.setText("Le mois est invalide !");
                         return;
                     }else if(!((yeartxt.getText()).matches("1[8-9]|2[0-9]|3[0-9]|4[0-9]|5[0-9]"))){
                         warning.setText("L'année est invalide !");
                         return;
                     }else{
                     warning.setText("");
                     ...
                     infoBox1("Paiement avec succès !", null, "sucess");
                     }
                     ...
                     infoBox1("Entrer les détails de paiement !",null,"Alert!");
                 }
             }else{
                 paymentModel.update_payment_status_to_confirmed();
                 paymentModel.update_status_to_payment_confirmed();
                 infoBox1("Demande confirmée !",null, "Success");
                  try {
                         Node node = (Node)event.getSource();
                         Stage dialogStage = (Stage) node.getScene().getWindow();
                         dialogStage.close();
